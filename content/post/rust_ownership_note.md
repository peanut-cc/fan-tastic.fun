...
Rust 中大括号就表示一个单独的作用域。所以常见的大括号作用域有：
...
![变量位置和值](/images/rust_base_data_type/变量位置和值.png)
...
![变量位置和值2](/images/rust_base_data_type/变量位置和值2.png)
...
![变量位置和值3](/images/rust_base_data_type/变量位置和值3.png)
...
- 一个值同时只能被一个变量所拥有，或者说一个值只能拥有一个所有者。(值在任一时刻有且只有一个所有者)
...
![string内存结构](/images/rust_owership/string内存结构.png)
...
s 是 String 字符串，变量 s 是指向 栈上的胖指针，在执行 `let s2 = s;` 的时候，将会拷贝 s 在栈上的胖指针，然后让s 无效，所有权转移之后，将只有新的所有者才会指向堆中的实际数据。
...
这里因为字符串字面量 "fan-tasitc" 是在 二进制文件的 `.RODATA`段中 中，所以 x 只是引用了存储在二进制文件的 `.RODATA`段的字符串，并未持有所有权。所以`let y = x;` 知识对该引用进行了拷贝，此时 y 和 x 都引用了同一个字符串。
...
个人理解 “在有可变引用时，不允许存在其他可变引用和不可变应用” 的意思是：

- 不可变引用是可以和可变引用同时创建。
- 如果存在可变引用，这个时候就会具有排他性，但这个拍他性在于你是否同时使用。
